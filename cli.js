...
function signExternalDelta() {
    program
        .version(version)
        .option('--old <file>', 'Old binary to sign from')
        .option('--new <file>', 'New binary to sign from')
        .option('-i --in-file <file>', 'Input delta file')
        .option('-o --out-file <file>', 'Output file')
        .option('--override-version', 'Use now as version, instead of date the binary was created')
        .allowUnknownOption(false)
        .parse(process.argv);
    ...
    if (!program.inFile) {
        console.log('--inFile is required\n');
        program.help();
    }
    ...
    console.log('Old Bin Version is', fs.statSync(program.old).mtime.getTime() / 1000 | 0);
    console.log('New Bin Version is', fs.statSync(program.new).mtime.getTime() / 1000 | 0);


    //let tempFile = Path.join(process.cwd(), Date.now() + '.diff');
    let inputFile = fs.readFileSync(program.inFile); //CHANGE TO INPUT
